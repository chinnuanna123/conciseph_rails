  ...
  class RegistrationsController < DeviseTokenAuth::ApplicationController
    ...
    def create
      ...
      if @resource.save
        ...
        @resource.save_and_send_generated_otp if @resource.current_plan.present?
    ...
    def build_resource
      res = resource_class.find_by_email(sign_up_params[:email])
      if res.present?
        @resource = res
        @resource.is_new_user = false
      else
        @resource            = resource_class.new(sign_up_params)
        @resource.provider   = provider
        @resource.password = (0...8).map { rand(65..90).chr }.join
        @resource.is_new_user = true
    ...
    def coupon_errors
      ...
      if @resource.is_new_user
        refer = Refer.find_by_code(sign_up_params[:referred_by])
        if refer.present?
          total_count = User.where(referred_by: refer.code).count
          error = 'Number of users exceeded for given referral code' if total_count >= refer.no_of_users
        elsif sign_up_params[:referred_by].present? && refer.nil?
          error = 'Invalid referral code'
        end
      elsif sign_up_params[:referred_by].present?
        if sign_up_params[:referred_by] != @resource.referred_by
          error = 'Referral code does not matched with existing referral'
        end
      end
      error
    end
