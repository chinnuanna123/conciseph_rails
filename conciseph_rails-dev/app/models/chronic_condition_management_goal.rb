...
class ChronicConditionManagementGoal < ApplicationRecord
  ...
  validates :name, :chronic_condition_management_goal_type, :chronic_condition_management_goal_category, :status,
            :start_date, :end_date, :section, presence: true
  ...
  enum chronic_condition_management_goal_category: {
    'Diabetic': 0,
  ...
  enum chronic_condition_management_goal_type: {
    'Quality Measures': 0
  }
  ...
  def self.search(params)
    ...
    query = query.where(chronic_condition_management_goal_category: category) if category.present?
